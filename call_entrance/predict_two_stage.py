...
【优化说明】集成了 predict_complete.py 的以下优化：
  ✅ 框有效性检查 (尺寸、纵横比)
  ✅ 贴边框自动扩展逻辑 (expand_ratio=0.15)
  ✅ 无效框过滤 (大小、纵横比、贴边等)
...
def predict_fuel_with_rotation_tta(model, crop_bgr: np.ndarray, device: torch.device, use_tta: bool = True):
    """ResNet rotation TTA: average fuel prediction over 0/90/180/270."""
    preds = []
    angles = (0, 90, 180, 270) if use_tta else (0,)
    with torch.no_grad():
        for angle in angles:
    ...
    for img_path in image_files:
        ...
        if is_edge_touch:
            x1_exp, y1_exp, x2_exp, y2_exp = expand_box_if_edge_touching(
                x1, y1, x2, y2, h, w, expand_ratio=0.15
            )
