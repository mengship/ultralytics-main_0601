...
# -*- coding: utf-8 -*-
"""
【两阶段两模型训练 v2修复版】YOLO分类 + ResNet双模型油量识别
...
【v2修复】
✅ 修正样本权重（格子类4.6倍权重，解决类别不平衡）
✅ 优化损失函数（改用SmoothL1Loss，训练更稳定）
✅ 删除翻转增强（避免非对称油表视觉混淆）
✅ 保留旋转增强（-30°~+30°，适应实际拍照场景）
...
class FuelTypeDataset(Dataset):
    ...
    def _augment(self, img):
        ...
        if np.random.rand() < (0.4 if is_grid else 0.3):
            scale = np.random.uniform(0.95 if is_grid else 0.95, 1.05 if is_grid else 1.05)
        ...
        # ❌ v2修复：删除水平翻转（避免非对称油表视觉混淆）
        # if np.random.rand() < (0.5 if is_grid else 0.2):
        #     img = cv2.flip(img, 1)

        # ❌ v2修复：删除竖直翻转（避免非对称油表视觉混淆）
        # if np.random.rand() < (0.5 if is_grid else 0.2):
        #     img = cv2.flip(img, 0)
        ...
        # 8. 随机仿射变换（格子类专用，降低概率）
        if is_grid and np.random.rand() < 0.1:  # v2修复：从0.3降到0.1
...
def train_resnet_for_fuel_type(fuel_type, fuel_type_name, detection_dataset_dir="fuel_detection_dataset"):
    ...
    # ⭐ v2修复：根据类别数量计算真实权重（解决类别不平衡）
    # 指针类495张 vs 格子类107张 = 4.6:1
    num_samples = len(train_dataset)
    weights = []

    for i in range(num_samples):
        img_name = train_dataset.image_files[i].stem
        fuel_type_str = train_dataset.type_data.get(img_name)
    ...
    # ✅ v2修复：改用SmoothL1Loss（结合L1和L2优点，训练更稳定）
    ...
    print(f"   - weight_decay: 3e-4 ✅")
    print(f"   - Dropout: 0.5/0.4/0.3 ✅")
    print(f"   - 损失函数: SmoothL1Loss(beta=0.1) ✅ (v2修复)")
    print(f"   - 样本权重: 格子类4.6倍 ✅ (v2修复)")
    print(f"   - 数据增强: 删除翻转 ✅ (v2修复)")
    if freeze_warmup_epochs > 0:
        print(f"   - 冻结热身轮次: {freeze_warmup_epochs} ✅（大样本策略）")
    else:
        print(f"   - 冻结热身轮次: 禁用 ✅（小样本策略）")
    print(f"   - drop_last: True ✅")
    print(f"   - WeightedRandomSampler: True ✅")
