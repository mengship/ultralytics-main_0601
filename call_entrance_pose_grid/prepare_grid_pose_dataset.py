#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""生成格子油表 YOLO Pose 数据集

从 LabelMe JSON 标注中提取格子油表的关键点，生成 YOLO Pose 数据集。
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="生成格子油表 YOLO Pose 数据集"
    )
    parser.add_argument(
        "--source-dir",
    ...
    parser.add_argument(
        "--output-dir",
        default="call_entrance_pose_grid/dataset_grid_pose",
        help="输出数据集目录",
    )
    parser.add_argument(
        "--val-ratio",
...
def extract_grid_pose_data(
    json_path: Path,
) -> Optional[Dict]:
    """从 JSON 中提取格子油表的框和关键点
    ...
    oil1_box = None
    keypoints = {'empty': None, 'full': None, 'tip': None}
    ...
    for shape in shapes:
        ...
        if label == 'oil1':
            if shape_type == 'rectangle' and len(points) >= 2:
                oil1_box = rectangle_to_bbox(points)
            elif shape_type == 'rotation' and len(points) >= 4:
                oil1_box = rotation_to_bbox(points)
...
def process_dataset(
    ...
    # 只保留格子油表样本
    df_grid = df[df['油表类型'] == '格子']
    print(f"格子油表样本: {len(df_grid)}")
    ...
    for idx, row in df_grid.iterrows():
        ...
        # 提取格子油表数据
        result = extract_grid_pose_data(json_path)
        if result is None:
            # 进一步判断缺失原因
            try:
                with open(json_path, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    shapes = data.get('shapes', [])
                    has_oil1 = any(s.get('label') == 'oil1' for s in shapes)

                    if not has_oil1:
                        missing_report['missing_oil1'].append(image_name)
                    else:
    ...
    print(f"  缺失 oil1 框: {len(missing_report['missing_oil1'])}")
